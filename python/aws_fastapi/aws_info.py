...
import configparser, argparse, datetime, inspect,sys
...
def ec2_info_get():
  ...
  for n in ['ec2_data_linux', 'ec2_data_win', 'ec2_ri_buy']:
    if n in locals():
      v = locals()[n]
      p(n, v)
  return ec2_data_linux, ec2_data_win, ec2_ri_buy

# def ec2_data_linux_get():
#   ec2_infos, ec2_infos_change = ec2_info_new(profile)
...
def rds_info_get():
  ...
  for n in ['rds_data', 'ri_rds_infos', 'rds_ri_buy']:
    if n in locals():
      v = locals()[n]
      p(n, v)
  return rds_data, ri_rds_infos, rds_ri_buy
...
def redis_info_get():
  ...
  for n in ['redis_data', 'ri_redis_infos', 'redis_ri_buy']:
    if n in locals():
      v = locals()[n]
      p(n, v)
  return redis_data, ri_redis_infos, redis_ri_buy
...
def opensearch_info_get():
  ...
  for n in ['es_data', 'ri_es_infos', 'es_ri_buy']:
    if n in locals():
      v = locals()[n]
      p(n, v)
  return es_data, ri_es_infos, es_ri_buy

def all_info_get():
  ec2_info = ec2_info_get()['ec2_ri_buy']
  rds_info = rds_info_get()['rds_ri_buy']
  redis_info = redis_info_get()['redis_ri_buy']
  es_info = opensearch_info_get()['es_ri_buy']
...
# @app.get("/ec2",
#   summary='EC2 & EC2_RI Infos',
#   description='Get All EC2 & EC2_RI Infos',
#   response_description='response',
#   tags=['ec2'])
# async def get_ec2_info():
#   ec2_data_linux, ec2_data_win, ec2_ri_buy = rds_info_get()
#   tmp = {}
#   for n in ['ec2_data_linux', 'ec2_data_win', 'ec2_ri_buy']:
#     tmp[n] = dict(locals()[n])
#   data = {'ec2': tmp}
...
async def get_ec2_info(ec2_data: ec2_data):
  ...
  # if ec2_data == "linux":
  #   return data['ec2']["ec2_data_linux"]

  # if ec2_data == "win":
  #   return data['ec2']["ec2_data_win"]

  # if ec2_data == "ri":
  #   return data['ec2']["ec2_ri_buy"]
  def f(ec2_data):
    return {
      'all': data,
...
async def es():
  ...
  for n in ['es_data', 'ri_es_infos', 'es_ri_buy']:
    tmp[n] = dict(locals()[n])
  data = {'elaticache': tmp}
  # data = {'elaticache': {"es_data": dict(es_data), "ri_es_infos": dict(ri_es_infos), "es_ri_buy": dict(es_ri_buy)}}
  return data
...
# print(type(ec2_info_new(profile)))